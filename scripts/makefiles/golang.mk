...
go.test: ## 执行单元测试.
	...
	@gsed -i '/mock_*.go/d' $(OUTPUT_DIR)/coverage.out # 从 coverage 中删除mock_.*.go 文件 MacOS下`brew install gnu-sed`
	@gsed -i '/internal\/torchpole\/store\/.*.go/d' $(OUTPUT_DIR)/coverage.out # internal/torchpole/store/ 下的 Go 代码不参与覆盖率计算

.PHONY: go.cover
go.cover: go.test ## 执行单元测试，并校验覆盖率阈值.
	@$(GO) tool cover -func=$(OUTPUT_DIR)/coverage.out | gawk -v target=$(COVERAGE) -f $(ROOT_DIR)/scripts/coverage.awk # MacOS下 brew install gawk
