...
gen.ca: ## 生成 CA 文件.
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/ca.key -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "/C=CN/ST=ShangHai/L=ShangHai/O=clientrppkg/OU=clientit/CN=127.0.0.1/emailAddress=dairongpeng@foxmail.com" # 2. 生成请求文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "/C=CN/ST=ShangHai/L=ShangHai/O=serverrppkg/OU=serverit/CN=127.0.0.1/emailAddress=dairongpeng@foxmail.com" # 6. 生成服务端向 CA 申请签名的 CSR
